(function() {
	...
	angular
		.module('wi.role.grid')
		.controller('WiRoleGridCtr', WiRoleGridCtr);
	
	WiRoleGridCtr.$inject = ['$scope', 'cfg', 'acl', '$q', '$state', '$stateParams', 'RoleResource', 'uiGridConstants', '$uibModal'];		//~
	
	function WiRoleGridCtr($scope, cfg, acl, $q, $state, $stateParams, Resource, uiGridConstants, $uibModal) {		//~
		/* jshing validthis:true */
		var vm = this;
    	...
    	$scope.columns = [
    		...
    		{name: 'Rol', field: 'role', visible: true, enableSorting: true, cellTemplate: cellTemplate}
    	 ...
    	 */
    	function create() {
    		$state.go('main.role.edit', {roleId: 'new'});	//~
    	} // create
    	...
    	function rowClick(row) {
    		// Go to edit row
    		$state.go('main.role.edit', {roleId: row.entity.id});
    	} // rowClick
