...
(function() {
	...
	angular
		.module('wi.role.form')
		.controller('WiRoleFormCtr', WiRoleFormCtr);
	
	WiRoleFormCtr.$inject = ['$scope', 'acl', '$q', '$state', '$stateParams', 'RoleResource'];	//~
	
	function WiRoleFormCtr($scope, acl, $q, $state, $stateParams, Resource) {		//~
    	...
    	function activate() {
    		...
    		// Otherwise load existing entity
    			...
    			// If id is valid	//~
    			if (angular.isNumber(parseInt($stateParams.roleId))) {
    	...
    	function loadEntity() {
    		return $q(function(resolve, reject) {
        		...
        		Resource.get({id: $stateParams.roleId})
        			// On success
    		...
    		return $q(function(resolve, reject) {
    			...
    			Resource.save(vm.entity)    			
        			...
        			.$promise.then(function(data) {
        				...
        				$state.transitionTo('main.role.edit', {roleId: vm.entity.id}, { notify: false });
        				// Resolve
        				resolve(data);
